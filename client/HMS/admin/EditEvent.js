...
const EditEvent = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!eventName || !eventDate || !eventLocation || !eventDescription  || !eventEndTime || !eventTime) {
      Alert.alert('Please fill in all fields');
      return;
    } else {
      try {
        const response = await axios.post('/auth/EditEvent', eventData);
        ...
        if (error.response && error.response.data.message === 'room with the same name already exists') {
          Alert.alert('room with the same name already exists');
        } else {
          Alert.alert('room creating event: ' + error.message);
        }
      }
    }
  };
